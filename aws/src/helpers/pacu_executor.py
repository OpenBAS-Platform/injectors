...
class PacuExecutor:
        ...
        try:
            ...
            if result.returncode < 0:
                ...
                return {
                    "success": False,
                    "module": module_name,
                    "error": f"{signal_name}: {error_detail}" if error_detail else f"Pacu terminated by {signal_name}",
                }
    ...
    def execute_iam_create_user(self, username: str) -> Dict:
        ...
        if self.logger:
            self.logger.info(f"Executing IAM create user attack for username: {username}")
    ...
    def execute_aws_cli_command(self, command: str = "s3 ls") -> Dict:
        ...
        try:
            ...
            if "s3 ls" in command:
                ...
                username = command.split("--user-name")[-1].strip() if "--user-name" in command else "unknown"
    ...
    def parse_results(self, results: Dict) -> Dict:
        ...
        if not results.get("success"):
            ...
            if "ImportError" in error_msg or "cannot import" in error_msg:
                ...
                simple_error = error_msg.replace("\\", "/").replace("\n", " ").strip()[:100]
        ...
        # Parse IAM password policy (AWS CLI)
            ...
            try:
                ...
                if "MinimumPasswordLength" in password_policy:
                    policy_items.append(f"Minimum Length: {password_policy['MinimumPasswordLength']}")
                if "RequireSymbols" in password_policy:
                    policy_items.append(f"Require Symbols: {password_policy['RequireSymbols']}")
                if "RequireNumbers" in password_policy:
                    policy_items.append(f"Require Numbers: {password_policy['RequireNumbers']}")
                if "RequireUppercaseCharacters" in password_policy:
                    policy_items.append(f"Require Uppercase: {password_policy['RequireUppercaseCharacters']}")
                if "RequireLowercaseCharacters" in password_policy:
                    policy_items.append(f"Require Lowercase: {password_policy['RequireLowercaseCharacters']}")
                if "AllowUsersToChangePassword" in password_policy:
                    policy_items.append(f"Allow Users to Change Password: {password_policy['AllowUsersToChangePassword']}")
                if "ExpirePasswords" in password_policy:
                    policy_items.append(f"Password Expiration: {password_policy['ExpirePasswords']}")
                if "MaxPasswordAge" in password_policy:
                    policy_items.append(f"Max Password Age: {password_policy['MaxPasswordAge']} days")
                if "PasswordReusePrevention" in password_policy:
                    policy_items.append(f"Password Reuse Prevention: {password_policy['PasswordReusePrevention']} passwords")
        ...
        # Parse IAM create user (AWS CLI)
            ...
            try:
                ...
                if "EntityAlreadyExists" in output or "already exists" in output.lower():
                    message = "User already exists"
                elif "AccessDenied" in output:
                    message = "Access denied - insufficient permissions to create user"
                else:
                    message = f"Failed to create user: {str(e) if e else 'Unknown error'}"
    ...
    def _parse_iam_entities(self, stdout: str) -> Tuple[List[str], List[str], List[str], List[str]]:
    ...
    def _parse_iam_roles(self, stdout: str) -> List[str]:
        ...
        for line in stdout.split("\n"):
            ...
            if "arn:aws:iam::" in line and "role/" in line:
                ...
                role_name = line.split("role/")[1].split()[0] if "role/" in line else line
    ...
    def _parse_privesc_paths(self, stdout: str) -> List[str]:
        ...
        for line in stdout.split("\n"):
            ...
            if any(keyword in line.lower() for keyword in ["escalation", "privesc", "vulnerable", "exploit"]):
    ...
    def _parse_lambda_functions(self, stdout: str) -> List[str]:
        ...
        for line in stdout.split("\n"):
            ...
            if "arn:aws:lambda:" in line:
                ...
                func_name = line.split("function:")[1].split()[0] if "function:" in line else line
    ...
    def _generic_parse(self, module_name: str, stdout: str) -> Dict:
        ...
        if "rds" in module_name:
            outputs["databases"] = self._extract_items(stdout, ["DBInstanceIdentifier", "Database:", "RDS:"])
        elif "secrets" in module_name:
            outputs["secrets"] = self._extract_items(stdout, ["SecretName:", "Secret:", "ARN:"])
        elif "ssm" in module_name or "parameter" in module_name:
            outputs["parameters"] = self._extract_items(stdout, ["Parameter:", "Name:", "SSM:"])
        elif "vpc" in module_name:
            outputs["vpcs"] = self._extract_items(stdout, ["vpc-", "VPC:", "VpcId:"])
        elif "cloudtrail" in module_name:
            outputs["events"] = self._extract_items(stdout, ["Event:", "EventName:", "Trail:"])
        elif "organizations" in module_name:
            outputs["organizations"] = self._extract_items(stdout, ["Account:", "Organization:", "OrgId:"])
        elif "ebs" in module_name:
            outputs["snapshots"] = self._extract_items(stdout, ["snap-", "Snapshot:", "SnapshotId:"])
        elif "dynamodb" in module_name:
            outputs["tables"] = self._extract_items(stdout, ["Table:", "TableName:", "DynamoDB:"])
        elif "ecr" in module_name:
            outputs["repositories"] = self._extract_items(stdout, ["Repository:", "RepositoryName:", "ECR:"])
        elif "ecs" in module_name:
            outputs["clusters"] = self._extract_items(stdout, ["Cluster:", "ClusterArn:", "ECS:"])
        elif "eks" in module_name:
            outputs["clusters"] = self._extract_items(stdout, ["Cluster:", "ClusterName:", "EKS:"])
        elif "guardduty" in module_name:
            outputs["findings"] = self._extract_items(stdout, ["Finding:", "FindingId:", "GuardDuty:"])
        elif "cognito" in module_name:
            outputs["user_pools"] = self._extract_items(stdout, ["UserPool:", "PoolId:", "Cognito:"])
        elif "glue" in module_name:
            outputs["databases"] = self._extract_items(stdout, ["Database:", "DatabaseName:", "Glue:"])
        elif "route53" in module_name:
            outputs["hosted_zones"] = self._extract_items(stdout, ["HostedZone:", "Zone:", "Route53:"])
        elif "sns" in module_name:
            outputs["topics"] = self._extract_items(stdout, ["Topic:", "TopicArn:", "SNS:"])
        else:
            # Default: try to extract any identifiable items
            outputs["items"] = self._extract_items(stdout, ["Name:", "Id:", "ARN:", "Identifier:"])
        ...
        for line in text.split("\n"):
            ...
            for pattern in patterns:
                if pattern in line:
                    ...
                    if ":" in pattern:
                        value = line.split(pattern, 1)[1].strip() if pattern in line else ""
                    else:
