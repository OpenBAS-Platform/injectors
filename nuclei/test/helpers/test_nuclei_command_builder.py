from nuclei.src.contracts.constants import CLOUD_SCAN_CONTRACT, CVE_SCAN_CONTRACT, TEMPLATE_SCAN_CONTRACT, \
    HTTP_SCAN_CONTRACT
from nuclei.src.helpers.nuclei_command_builder import NucleiCommandBuilder

def test_cve_contract():
...
def test_template_contract_with_template_and_cve():
    builder = NucleiCommandBuilder()
    content = {"template": "cves/2021/1234.yaml"}
    targets = ["https://example.com"]
    args = builder.build_args(HTTP_SCAN_CONTRACT, content, targets)
    expected_args = ["nuclei", "-tags", "http", "-t", "cves/2021/1234.yaml", "-j", "-u", "https://example.com"]
    assert args == expected_args
